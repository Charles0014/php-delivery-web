...
class Command {
                ...
                case '--generate-configuration':
                    ...
                    \file_put_contents(
                            'phpunit.xml', $generator->generateDefaultConfiguration(
                                    Version::series(), $bootstrapScript, $testsDirectory, $src
                            )
                    );
    ...
    protected function handlePrinter($printerClass, $printerFile = '') {
        ...
        if (!\class_exists($printerClass)) {
            $this->exitWithErrorMessage(
                    \sprintf(
                            'Could not use "%s" as printer: class does not exist', $printerClass
                    )
            );
        }
        ...
        if (!$class->implementsInterface(TestListener::class)) {
            $this->exitWithErrorMessage(
                    \sprintf(
                            'Could not use "%s" as printer: class does not implement %s', $printerClass, TestListener::class
                    )
            );
        }
        ...
        if (!$class->isSubclassOf(Printer::class)) {
            $this->exitWithErrorMessage(
                    \sprintf(
                            'Could not use "%s" as printer: class does not extend %s', $printerClass, Printer::class
                    )
            );
        }
        ...
        if (!$class->isInstantiable()) {
            $this->exitWithErrorMessage(
                    \sprintf(
                            'Could not use "%s" as printer: class cannot be instantiated', $printerClass
                    )
            );
        }
    ...
    protected function handleVersionCheck() {
        ...
        if ($isOutdated) {
            \printf(
                    ...
                    'The latest version is PHPUnit %s.' . PHP_EOL, $latestVersion
            );
        } else {
...
EOT;
    ...
    private function handleListTestsXml(TestSuite $suite, string $target, bool $exit): int {
        ...
        \printf(
                'Wrote list of tests that would have been run to %s' . \PHP_EOL, $target
        );
