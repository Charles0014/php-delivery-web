...
class TraceableUrlMatcherTest extends TestCase {
    ...
    public function testMatchRouteOnMultipleHosts() {
        ...
        $routes->add('first', new Route(
                '/mypath/', ['_controller' => 'MainBundle:Info:first'], [], [], 'some.example.com'
        ));

        $routes->add('second', new Route(
                '/mypath/', ['_controller' => 'MainBundle:Info:second'], [], [], 'another.example.com'
        ));
        ...
        $this->assertSame(
                [TraceableUrlMatcher::ROUTE_ALMOST_MATCHES, TraceableUrlMatcher::ROUTE_ALMOST_MATCHES], $this->getLevels($traces)
        );
    }
