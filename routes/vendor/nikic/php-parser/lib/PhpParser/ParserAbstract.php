...
abstract class ParserAbstract implements Parser {
                ...
                if ($rule === 0) {
                    ...
                    switch ($this->errorState) {
                        ...
                        case 2:
                            ...
                            while (!(
                            (($idx = $this->actionBase[$state] + $this->errorSymbol) >= 0 && $idx < $this->actionTableSize && $this->actionCheck[$idx] === $this->errorSymbol) || ($state < $this->YY2TBLSTATE && ($idx = $this->actionBase[$state + $this->numNonLeafStates] + $this->errorSymbol) >= 0 && $idx < $this->actionTableSize && $this->actionCheck[$idx] === $this->errorSymbol)
    ...
    string $string, int $indentLen, string $indentChar, bool $newlineAtStart, bool $newlineAtEnd, array $attributes
    ) {
        if ($indentLen === 0) {
            return $string;
        }
        ...
        return preg_replace_callback(
                $regex, function ($matches) use ($indentLen, $indentChar, $attributes) {
            ...
            if (false !== strpos($prefix, $indentChar === " " ? "\t" : " ")) {
                ...
                $this->emitError(new Error(
                        ...
                        '(expecting an indentation level of at least ' . $indentLen . ')', $attributes
                ));
            }
    ...
    string $startToken, $contents, string $endToken, array $attributes, array $endTokenAttributes, bool $parseUnicodeEscape
    ) {
        ...
        if ($indentHasSpaces && $indentHasTabs) {
            $this->emitError(new Error(
                    'Invalid indentation - tabs and spaces cannot be mixed', $endTokenAttributes
            ));
            ...
            foreach ($contents as $i => $part) {
                if ($part instanceof Node\Scalar\EncapsedStringPart) {
                    ...
                    $part->value = $this->stripIndentation(
                            $part->value, $indentLen, $indentChar, $i === 0, $isLast, $part->getAttributes()
                    );
