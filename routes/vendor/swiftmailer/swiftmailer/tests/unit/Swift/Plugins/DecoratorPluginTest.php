...
class Swift_Plugins_DecoratorPluginTest extends \SwiftMailerTestCase {
    ...
    public function testMessageBodyReceivesReplacements() {
        $message = $this->createMessage(
                $this->createHeaders(), ['zip@button.tld' => 'Zipathon'], ['chris.corbyn@swiftmailer.org' => 'Chris'], 'Subject', 'Hello {name}, you are customer #{id}'
        );
    ...
    public function testReplacementsCanBeAppliedToSameMessageMultipleTimes() {
        $message = $this->createMessage(
                $this->createHeaders(), ['zip@button.tld' => 'Zipathon', 'foo@bar.tld' => 'Foo'], ['chris.corbyn@swiftmailer.org' => 'Chris'], 'Subject', 'Hello {name}, you are customer #{id}'
        );
        $message->shouldReceive('setBody')
                ->once()
                ->with('Hello Zip, you are customer #456');
    ...
    public function testReplacementsCanBeMadeInHeaders() {
        ...
        $message = $this->createMessage(
                $headers, ['zip@button.tld' => 'Zipathon'], ['chris.corbyn@swiftmailer.org' => 'Chris'], 'A message for {name}!', 'Hello {name}, you are customer #{id}'
        );
    ...
    public function testReplacementsAreMadeOnSubparts() {
        ...
        $message = $this->createMessage(
                $this->createHeaders(), ['zip@button.tld' => 'Zipathon'], ['chris.corbyn@swiftmailer.org' => 'Chris'], 'A message for {name}!', 'Subject'
        );
    ...
    public function testReplacementsCanBeTakenFromCustomReplacementsObject() {
        $message = $this->createMessage(
                $this->createHeaders(), ['foo@bar' => 'Foobar', 'zip@zap' => 'Zip zap'], ['chris.corbyn@swiftmailer.org' => 'Chris'], 'Subject', 'Something {a}'
        );
