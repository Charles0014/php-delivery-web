...
class Swift_Signers_SMimeSigner implements Swift_Signers_BodySigner {
    ...
    protected function smimeSignMessage(Swift_Message $message) {
        ...
        if (!openssl_pkcs7_sign(
                        $messageStream->getPath(), $signedMessageStream->getPath(), $this->signCertificate, $this->signPrivateKey, [], $this->signOptions, $this->extraCerts
                )
        ) {
    ...
    protected function smimeEncryptMessage(Swift_Message $message) {
        ...
        if (!openssl_pkcs7_encrypt(
                        $messageStream->getPath(), $encryptedMessageStream->getPath(), $this->encryptCert, [], 0, $this->encryptCipher
                )
        ) {
    ...
    Swift_Message $fromMessage, Swift_Message $toMessage, array $headers = []
    ) {
        ...
        switch ($header->getFieldType()) {
            ...
            case Swift_Mime_Header::TYPE_PARAMETERIZED:
                $headers->addParameterizedHeader(
                        $header->getFieldName(), $header->getValue(), $header->getParameters()
                );
                break;
        }
    }
