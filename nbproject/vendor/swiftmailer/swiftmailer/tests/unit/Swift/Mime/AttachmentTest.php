...
class Swift_Mime_AttachmentTest extends Swift_Mime_AbstractMimeEntityTest {
    ...
    public function testDispositionIsReturnedFromHeader() {
        ...
        $attachment = $this->createAttachment($this->createHeaderSet([
                    'Content-Disposition' => $disposition,]), $this->createEncoder(), $this->createCache()
        );
    ...
    public function testDefaultContentTypeInitializedToOctetStream() {
        ...
        $attachment = $this->createAttachment($this->createHeaderSet([
                    'Content-Type' => $cType,]), $this->createEncoder(), $this->createCache()
        );
    }
    ...
    public function testFilenameIsReturnedFromHeader() {
        ...
        $disposition = $this->createHeader('Content-Disposition', 'attachment', ['filename' => 'foo.txt']
        );
        $attachment = $this->createAttachment($this->createHeaderSet([
                    'Content-Disposition' => $disposition,]), $this->createEncoder(), $this->createCache()
        );
    ...
    public function testFilenameIsSetInHeader() {
        $disposition = $this->createHeader('Content-Disposition', 'attachment', ['filename' => 'foo.txt'], false
        );
    ...
    public function testSizeIsReturnedFromHeader() {
        ...
        $disposition = $this->createHeader('Content-Disposition', 'attachment', ['size' => 1234]
        );
        $attachment = $this->createAttachment($this->createHeaderSet([
                    'Content-Disposition' => $disposition,]), $this->createEncoder(), $this->createCache()
        );
    ...
    public function XtestContentTypeCanBeLookedUpFromCommonListIfNotProvided() {
        ...
        $disposition = $this->createHeader('Content-Disposition', 'attachment', ['filename' => 'foo.zip'], false
        );
        $disposition->shouldReceive('setParameter')
                ->once()
        ...
        $attachment = $this->createAttachment($headers, $this->createEncoder(), $this->createCache(), ['zip' => 'application/zip', 'txt' => 'text/plain']
        );
        $attachment->setFile($file);
    }
