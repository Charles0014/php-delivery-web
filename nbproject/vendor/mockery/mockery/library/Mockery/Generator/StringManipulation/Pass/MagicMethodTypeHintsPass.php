...
class MagicMethodTypeHintsPass implements Pass {
    ...
    private function applyMagicTypeHints($code, Method $method) {
        if ($this->isMethodWithinCode($code, $method)) {
            ...
            $code = preg_replace(
                    $this->getDeclarationRegex($method->getName()), $this->getMethodDeclaration($method, $namedParameters), $code
            );
        }
        return $code;
    }
    ...
    private function isMethodWithinCode($code, Method $method) {
        return preg_match(
                        $this->getDeclarationRegex($method->getName()), $code
                ) == 1;
    }
    ...
    private function getOriginalParameters($code, Method $method) {
        ...
        preg_match(
                $this->getDeclarationRegex($method->getName()), $code, $matches
        );

        if (count($matches) > 0) {
            preg_match_all(
                    '/(?<=\$)(\w+)+/i', $matches[0], $parameterMatches
            );
        }
    ...
    Method $method, array $namedParameters
    ) {
        $declaration = 'public';
